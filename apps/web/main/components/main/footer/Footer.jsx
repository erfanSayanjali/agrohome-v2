...
const Footer = ({ data }) => {
    ...
    const text = data?.content?.items?.find(item => item?.widgetType === 'text')?.content || ''
    const lists = data?.content?.items?.filter(item => item?.widgetType !== 'text') || []



    return (
        <div className='w-full bg-[#103631f6] relative overflow-hidden '>
            <Image src={'/footers.png'} width={1000} height={500} className='absolute object-cover -z-50 bottom-0 w-full h-[900px]' alt='footer-shape' />
            <div className='max-w-7xl flex flex-col lg:flex-row justify-between lg:mx-auto mx-3 py-20'>
                <div className='flex md:w-[400px] flex-col gap-2'>
                    <Image
                        className='w-[140px] '
                        src={'/logo.png'} width={150} height={100} alt='logo-footer' />
                    <p className='text-sm text-gray-300 max-w-md mt-y text-justify'>
                        {
                            text
                        }
                    </p>
                    <p className='text-[#F7DB5E]'>
                        شبکه‌های اجتماعی
                    </p>
                    <div className='flex mt-1 items-center gap-3 text-white text-2xl'>
                        <FaInstagram className='box-content p-2 border border-white rounded-full' />
                        <PiTelegramLogoLight className='box-content p-2 border border-white rounded-full' />
                    </div>
                </div>
                                                ...
                                                items.length ?
                                                    items.map((item,i) => {
                                                        return (
                                                                ...
                                                                <LiItem key={i} href={item.link} title={item.title} />
