...
function BlogPage({blog , Suggestions}) {
  const suggestionSlides = (Suggestions || []).map((item) => (
    <BlogCard data={item} key={item._id || item.id || item.slug} />
  ))
  ...
  return (
    <div className='min-h-screen max-w-7xl lg:mx-auto md:mx-3 md:my-10'>
      <PagesTitle
        ...
        alt={blog?.title || 'بلاگ'}
        title={
          <h1 className="relative z-10 line-clamp-3 text-base leading-snug sm:text-lg md:text-2xl">
            {blog?.title}
          </h1>
        }
      />
      <div className="flex flex-col lg:flex-row items-stretch lg:items-start gap-5 lg:gap-7 px-3 md:px-0">
        <div className="w-full min-w-0 lg:w-[calc(100%-310px)]">
          <MediaTitle
            direction='vertical'
            wrapperClassName="w-full"
            data={{
              description: {
                className: 'flex flex-wrap items-center gap-3 sm:gap-4 text-sm sm:text-base',
                content: <>
              ...
              mediaElement: {
                className: '!w-full max-w-full rounded-2xl md:rounded-3xl object-cover aspect-[16/9] md:aspect-[4/1.5]',
                component: Image,
                props: {
                  alt: blog?.title || 'بلاگ',
          ...
          <div
            ...
            className="editor mt-3 max-w-full overflow-x-auto break-words"
          />
          <BlogComments blog_id={blog?._id || blog?.id}/>
        </div>
        <aside className="w-full lg:w-[310px] lg:shrink-0 lg:sticky top-5 flex flex-col gap-4">
          <div className="hidden lg:block">
            <ContentList />
          </div>
          {suggestionSlides.length ? (
            <div className="bg-[#F3F3F3] rounded-2xl p-4 w-full min-w-0 overflow-hidden">
              <p className="mb-3 font-extrabold">پیشنهاد مطالعه</p>
              <Slider
                className="!h-auto min-h-[390px]"
                gap={16}
                width={260}
                slides={suggestionSlides}
              />
            </div>
          ) : null}
        </aside>
